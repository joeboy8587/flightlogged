...
export const getCanonicalOperators = createServerFn({ method: "GET" }).handler(async (): Promise<CanonicalOperator[]> => {
  ...
  const MED_RX = /\b(MERCY|MEDIVAC|MEDEVAC|MED ?FLIGHT|AIR ?MED|AIRMED|LIFEFLIGHT|LIFE ?FLIGHT|REACH AIR|CALSTAR|MEDICAL|HOSPITAL|EMS|AMR|GUARDIAN|ANGEL|SHANDS|AIR EVAC|AIR METHODS)\b/i;
